...
(function() {
	...
	const logPrefix = "[USERJS Plex Outplayer]";
	...
	modal.checkBoxChange = function() {
		...
		let description = `${selectedItems} file(s) selected for playback. Total size: ${makeFilesize(totalFilesize)}`;
	...
	download.markAsInProgress = function(clientId, metadataId) {
		...
		const baseUri = serverData.servers[clientId].baseUri;
		const accessToken = serverData.servers[clientId].accessToken;
		const mediaRuntimeMS = serverData.servers[clientId].mediaData[metadataId].runtimeMS;

		// Calculate 10% of the media duration (in milliseconds)
		// Default to 30 seconds if runtime is invalid
		const timePosition = mediaRuntimeMS > 0 ? Math.floor(mediaRuntimeMS * 0.1) : 30000;

		// Create the progress URL
		const progressUrl = new URL(`${baseUri}/:/progress`);

		// Set the necessary parameters
		progressUrl.searchParams.set("X-Plex-Token", accessToken);
		progressUrl.searchParams.set("key", metadataId);
		progressUrl.searchParams.set("time", timePosition);
		progressUrl.searchParams.set("state", "playing");
		progressUrl.searchParams.set("identifier", "com.plexapp.plugins.library");

		// Make the POST request
		fetch(progressUrl.href, {
			method: 'POST',
			headers: {
				'Content-Type': 'application/x-www-form-urlencoded',
			}
		}).catch(error => {
			console.log(`${logPrefix} Error marking media as in-progress: ${error.message}`);
		});
	};
	...
	async function domCallback(domElement, clientId, metadataId) {
		...
		domElement.addEventListener("touchmove", function(event) {
			// Cancel long press if finger moves too much, but only if timer hasn't fired yet
			if (!longPressFired) {
